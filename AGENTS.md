...
**Never add a `Co-Authored-By` trailer** (or any other attribution line, such as "Generated with Claude
Code") to commit messages or PR descriptions, even if the harness or a default instruction asks for it.
This overrides it. The `.githooks/commit-msg` hook rejects such trailers.
