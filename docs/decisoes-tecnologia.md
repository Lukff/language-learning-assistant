...
| 28/08/2026 | Vídeo passou a ser servido por um `http.Server` real em `127.0.0.1` (`services/video_server.go`, `VideoServerService`), fora do AssetServer/scheme `wails://` do Wails v3, em todas as plataformas. Motivo: no Linux (WebKitGTK/GTK4), o scheme de URI customizado do Wails não entrega direito respostas 206/Range pro pipeline de mídia do GStreamer — o vídeo simplesmente não tocava, apesar do endpoint (`http.ServeFile`, suporte a range de graça) estar correto e testado. HTTP de loopback de verdade é o caminho que qualquer engine de vídeo sabe streamar nativamente, então deixou de valer a pena depender do scheme customizado do framework pra esse caso específico — sem violar o princípio da camada fina, já que `video_server.go` não importa nada do Wails, só é registrado como `application.Service` em `main.go`. Ver `docs/fase-1-mvp.md`, registro de 28/08/2026, pro diagnóstico completo. |
