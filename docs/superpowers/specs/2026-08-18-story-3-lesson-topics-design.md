...
> Covers Story 3 from `docs/phase-2-llm-analysis.md` (`analyze_topics`). Depends on Story 2
...
- A background job for `analyze_topics` (the "Promoção a job em background" criterion — only after
  the decision in `docs/llm-analysis-notes.md`).
...
(keep/refine/discard) in `docs/llm-analysis-notes.md` — **the first task without Phase 0
