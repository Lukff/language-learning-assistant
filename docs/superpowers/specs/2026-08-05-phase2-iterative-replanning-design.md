...
> This is not an implementation story — it's a replanning of `docs/phase-2-llm-analysis.md` itself.
...
`docs/superpowers/specs/2026-07-30-phase2-story-1-prompts-persistence-design.md`), but the work
was paused before the last acceptance criterion — validating the 7 tasks against a real lesson via
`cmd/validate-analysis` and recording the findings in `docs/llm-analysis-notes.md` — in order to
...
- **Validation now happens per task, visually in the UI — not in bulk via CLI.** Story 1's last
  ...
  `docs/llm-analysis-notes.md`) is replaced: each task is validated once it gets its own UI.
...
- Explicit decision criterion that closes the story: after observing the result in a few real
  lessons, record in `docs/llm-analysis-notes.md` whether the task is worth keeping as-is, needs
...
When a task is confirmed as valuable (a decision recorded in `docs/llm-analysis-notes.md`), a small
...
- **`docs/phase-2-llm-analysis.md`** is rewritten to reflect this structure: goal, technical risks
...
- **`docs/llm-analysis-notes.md`** doesn't change in this spec — it starts receiving one entry per
