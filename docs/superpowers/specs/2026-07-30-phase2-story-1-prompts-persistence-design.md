...
> Covers all of Story 1 (`docs/phase-2-llm-analysis.md`). Scope: prompts + task-agnostic `Provider`
...
against a real lesson (see `docs/llm-analysis-notes.md`). No code outside the package itself calls
...
2), a decision already recorded in `docs/phase-2-llm-analysis.md`: easier to refine prompt by
...
- **The manual-validation harness is a temporary CLI** (`cmd/validate-analysis`), the same
  ...
  lesson, the findings go into `docs/llm-analysis-notes.md`, and the CLI is removed — it doesn't
...
quality + real cost (tokens/USD) in `docs/llm-analysis-notes.md`, comparing against Phase 0's
single measurement. Only after that is the CLI removed.

## Acceptance criteria (from `docs/phase-2-llm-analysis.md`, Story 1)
...
- [ ] Validation on a real lesson: the 7 tasks run manually, result observed and recorded in
      `docs/llm-analysis-notes.md` (quality per task + real cost in tokens/USD).
