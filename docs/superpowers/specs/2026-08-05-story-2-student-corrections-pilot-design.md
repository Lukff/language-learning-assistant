...
> Covers the revised Story 2 from `docs/phase-2-llm-analysis.md` (the `analyze_corrections` pilot).
...
> (`docs/llm-analysis-notes.md`) and until each candidate task's turn comes.
...
`docs/superpowers/specs/2026-08-05-phase2-iterative-replanning-design.md`). This spec delivers
...
- A background job for `analyze_corrections` — only once the task is confirmed in
  `docs/llm-analysis-notes.md` (the "Promotion to background job" criterion from the phase doc).
...
## Acceptance criteria (from `docs/phase-2-llm-analysis.md`, Story 2)
...
- [ ] Decision recorded in `docs/llm-analysis-notes.md` after observing real lessons
