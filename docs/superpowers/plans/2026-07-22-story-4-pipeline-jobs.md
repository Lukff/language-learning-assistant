...
- `internal/` never imports Wails — thin layer (CLAUDE.md, `docs/technology-decisions.md`).
...
// worker decision in docs/technology-decisions.md and Story 4 in
// docs/phase-1-mvp.md. Status is always one of "pending", "running", "done",
// "error".
type Job struct {
	ID        int64
	LessonID  int64
...
// before a failing job becomes terminal "error" — see docs/phase-1-mvp.md
// (Story 4).
const maxAttempts = 3
...
verification already logged as pending for Stories 1 and 3 in `docs/phase-1-mvp.md`): run
...
same pending visual verification already noted in `docs/phase-1-mvp.md`, to be done whenever
