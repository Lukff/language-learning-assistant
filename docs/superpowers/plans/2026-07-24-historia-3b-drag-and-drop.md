...
- [ ] **Step 3: Confirm nothing else in the bindings tree changed unexpectedly**

  Run: `git status --short frontend/bindings/`
  Expected: only `importservice.ts` and `models.ts` show as modified (no unrelated churn from a stale generator run).

- [ ] **Step 4: Commit**

  ```bash
  git add frontend/bindings/assistente-idiomas/services/importservice.ts frontend/bindings/assistente-idiomas/services/models.ts
  git commit -m "chore: regenera bindings com ImportService.DropImport"
  ```
