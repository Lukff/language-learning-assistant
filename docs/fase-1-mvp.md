...
- [x] Janela abre de fato (verificação visual — `wails3 dev` ou o binário de `wails3 build`) nas máquinas Windows e Linux. Confirmado visualmente em ambas as máquinas.
...
- [x] Credenciais do provedor STT gravadas/lidas via `go-keyring`; nunca em texto plano. Cobertura unitária completa (round-trip + falha de keyring simulada); verificação manual em Windows e Linux (risco 3) confirmada.
...
- [x] A varredura roda automaticamente ao final do wizard de primeira execução (depois de escolher a pasta) e também fica disponível como ação sob demanda depois ("Sincronizar pasta"), para pegar vídeos jogados manualmente na pasta depois do setup. Verificado visualmente em Windows/Linux.
